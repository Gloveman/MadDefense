...
public class ItemControl2 : MonoBehaviourPun
{
    [SerializeField]
    AudioClip Itemsound;
    ...
    void Start()
    {
        ...
        Inventory[0] = 0;
        Inventory[1] = 0;
        Player2 = GameManager.instance.Player.GetComponent<Player2>();
    }
    ...
    void Update()
    {
        ...
        if (Input.GetButtonDown("Fire1") && (Player2.state != Player2.State.hurt) && Inventory[0] != 0)
        {
        ...
        if (Input.GetButtonDown("Fire2") && (Player2.state != Player2.State.hurt) && Inventory[1] != 0)
        {
            Debug.Log(999999999999999999);
